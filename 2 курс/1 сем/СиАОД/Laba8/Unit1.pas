...
type
    ...
    TForm1 = class(TForm)
        ...
        imgGraphView: TImage;
        strgrAdjTab: TStringGrid;
        seCountElem: TSpinEdit;
        lbCountElem: TLabel;
        btnGraphPaint: TButton;
        seFinishPoint: TSpinEdit;
        seStartPoint: TSpinEdit;
...
begin
  SetCountElem(seCountElem.Value);
end;
...
procedure TForm1.SetCountElem(value: integer);
var
  i,j:integer;
begin
  with strgrAdjTab do
...
procedure TForm1.seCountElemChange(Sender: TObject);
begin
  SetCountElem(seCountElem.Value);
  seStartPoint.MaxValue:=seCountElem.Value-1;
  seStartPoint.Value := 0;
  seFinishPoint.MaxValue:=seCountElem.Value-1;
  seFinishPoint.Value := seCountElem.Value-1;
end;
...
begin
  ...
  for i:=0 to CountElem-1 do
    ...
    try
      AdjTab[i,j] := StrToInt(strgrAdjTab.Cells[i,j]);
      if AdjTab[i,j] = 0 then  AdjTab[i,j] := Infinity;
    except
      //MessageBox(handle,PAnsiChar('Îøèáêà â ['+IntToStr(i)+','+IntToStr(j)+']'), PAnsiChar('Îøèáêà!'),MB_OK or MB_ICONERROR);
      Result := false;
      exit;
    end;
end;
...
begin
  ...
  ShortWay:=DeykstraAlgorithm(seStartPoint.Value,Way);
  if ShortWay[seFinishPoint.Value] >= Infinity then
  begin
    lbMinWay.Caption:= '(infinity)';
  end
  else
  begin
    WayName:=IntToStr(seFinishPoint.Value);
    i:=Way[seFinishPoint.Value];
    lbMinWay.Caption := '';
    while i <> seStartPoint.Value do
    begin
      WayName:=WayName + IntToStr(i);
      i:=Way[i];
    end;
    WayName := WayName + IntToStr(seStartPoint.Value);

    for i:= Length(WayName) downto 1 do
      lbMinWay.Caption:=lbMinWay.Caption + WayName[i] + ' ';
    lbMinWay.Caption:=lbMinWay.Caption +'(' +  IntToStr(ShortWay[seFinishPoint.Value]) + ')' ;
  end;
end;
...
begin
    Ways:=NewList;
    Src := seStartPoint.Value;
    Dest := seFinishPoint.Value;
    with NullWay do
    begin
        Name:= IntToStr(Src) + ' ';
        Cost:=0;
