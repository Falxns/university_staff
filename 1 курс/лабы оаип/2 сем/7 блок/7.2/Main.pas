...
begin
    MessageDlg('This program converts an incidence matrix to an incidence list.' + #13#10 + 'Vertices[2..9].' + #13#10 + 'Edges[1..9].' + #13#10 + 'In table cells use 0 or 1.', mtInformation, [mbOk], 0);
end;
...
begin
    ...
    ListF.ListSG.ColCount := StrToInt(EdgeEdit.Text) * 2 + 3;
    ListF.ListSG.RowCount := StrToInt(VertEdit.Text);
    ListF.ClientHeight := 33 * ListF.ListSG.RowCount;
    ListF.ClientWidth := 33 * ListF.ListSG.ColCount;
...
begin
    ...
    for j := 1 to SG.RowCount - 1 do
        for i := 1 to SG.ColCount - 1 do
            SG.Cells[i, j] := '';
    if (Length(VertEdit.Text) = 1)and(Length(EdgeEdit.Text) = 1) then
