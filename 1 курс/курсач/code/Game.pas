...
begin
    ...
    // добавление пули в массив
    inc(j);
    Bullets[j] := Obj;
    // задание анимации движения пули
    ...
    AnimY.Duration := 1;
    AnimY.Enabled := true;
    // удаление пули после окончания анимации
...
begin
    ...
    AnimX.Enabled := true;
    // анимация движения по вертикали
    AnimY := TFloatAnimation.Create(Obj);
    AnimY.Parent := Obj;
    AnimY.StartValue := Obj.Position.Y;
    AnimY.PropertyName := 'Position.Y';
...
begin
    ...
    for m := ii to jj do
        if (Balls[m] <> nil)and(Balls[m].Position.X + Balls[m].Width > Canon.Position.X + 20)and(Balls[m].Position.X < Canon.Position.X + Canon.Width - 20) then
            ...
            begin
                // отключение стрельбы и появления мишеней
                Ball.Enabled := false;
                Bullet.Enabled := false;
                Check.Enabled := false;
                // очистка игрового поля от шаров
                for l := 1 to 100 do
                    if Balls[l] <> nil then
                    begin
                        Balls[l].Destroy;
...
begin
    // проверка на возможность уничтожения цели
